...
def cases_detection(reachy):
    ...
    while True:
        ...
        # Sort the remaining rectangles by their y-coordinate (top-to-bottom)
        other_rectangles = sorted(rectangles[1:], key=lambda x: x[0][0][1])

        cases_coords = []
        for rect in other_rectangles:
            x, y, w, h = cv2.boundingRect(rect)
            cases_coords.append((x, y, x+w, y+h))

        # Draw rectangles from other_rectangles onto the imageFrame
        for rect in other_rectangles:
            x, y, w, h = cv2.boundingRect(rect)
    ...
    print("9 Case coords:", cases_coords)
